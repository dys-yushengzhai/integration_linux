...
def mixed_dataset(data='all',binary_weight=True,batch_size=1,n_max=5000,n_min=100):
    ...
    if data=='all':
        ...
        data_path_1 = 'data/data_all_v3_2D3D discretizations/'
        # data_path_2 = 'data/data_fe_triangulations/'
        if os.path.isfile(pickle_path+data+'.pickle'):
            ...
            # data_path_1     
            for m in os.listdir(os.path.expanduser(data_path_1)):  
                adj = mmread(os.path.expanduser(data_path_1+str(m)))
                if(isinstance(adj,numpy.ndarray)):
                    continue
                if adj.shape[0]<n_max and adj.shape[0]>n_min:
                    print(str(m)+"   "+str(number))
            ...
            # data_path_2
            # for m in os.listdir(os.path.expanduser(data_path_2)):  
            #     adj = mmread(os.path.expanduser(data_path_2+str(m)))
            #     if(isinstance(adj,numpy.ndarray)):
            #         continue
            #     if adj.shape[0]<n_max and adj.shape[0]>n_min:
            #         print(str(m)+"   "+str(number))
